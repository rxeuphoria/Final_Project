...
public class FinalApi {
	...
	public RecipesList[] showRecipesList(Double minCarbs, Double maxCarbs, Double minProtein, Double maxProtein,
		...
		URI url = UriComponentsBuilder.fromHttpUrl("https://api.spoonacular.com/recipes/findByNutrients")
				.queryParam("minCarbs", minCarbs).queryParam("maxCarbs", maxCarbs).queryParam("minProtein", minProtein)
				.queryParam("maxProtein", maxProtein).queryParam("minFats", minFats).queryParam("maxFats", maxFats)
				.queryParam("number", number).queryParam("apiKey", apiKey).build().toUri();
